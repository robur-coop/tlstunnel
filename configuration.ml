...
let decode_data data =
  ...
  | Ok snis ->
    List.fold_left
      (fun acc (sni, host, port) -> M.add sni (host, port) acc)
      M.empty snis
  | Error `Msg msg ->
...
let add_sni snis (sni, host, port) =
  (match M.find_opt sni snis with
   | None -> ()
   | Some (ohost, oport) ->
     Logs.warn (fun m -> m "overwriting %s -> %a:%d with %a:%d"
                   sni Ipaddr.pp ohost oport Ipaddr.pp host port));
  Logs.info (fun m -> m "%s is now redirected to %a:%d" sni Ipaddr.pp host port);
  M.add sni (host, port) snis

let remove_sni snis sni =
  Logs.info (fun m -> m "%s is no longer redirected" sni);
  M.remove sni snis

type cmd =
  | Add of string * Ipaddr.t * int
  | Remove of string
  | List
  | Snis of (string * Ipaddr.t * int) list
  | Result of int * string

let pp_one ppf (sni, host, port) =
    Fmt.pf ppf "%s -> %a:%u" sni Ipaddr.pp host port

let pp_cmd ppf = function
  | Add (s, h, p) -> Fmt.pf ppf "adding %a" pp_one (s, h, p)
  | Remove sni -> Fmt.pf ppf "removing %s" sni
